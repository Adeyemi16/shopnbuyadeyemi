...
import { getAllOrder, trackOrders } from "../../../config/api";
...
export default function Shoporders() {
  ...
  const handleOrderUpdate = async (id) => {
    ...
    try {
      const res = await trackOrders(id, { status: currentStatus + 1 }, currentUser.access_Token)
      ...
      {loading ? (
        ...
        <>
          {data?.length > 0 ? (
                  ...
                  <tr>
                    ...
                    <td
                      className={
                        ...
                        (order.status === 1 && "text-text-danger") ||
                        (order.status === 2 && "text-success")
                      }
                    >
            ...
            <h1 className="fs-6">No customer orders</h1>
          )}
        </>
      )}
    </>

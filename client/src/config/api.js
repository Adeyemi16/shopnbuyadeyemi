...
export const likeProduct = async (productId, userId, token) => {
    const res = await instance.put(`/api/v1/products/${productId}/like`, userId, { headers: { Authorization: `Bearer ${token}` }, })
    return res
}

export const disLikeProduct = async (productId, userId, token) => {
    const res = await instance.put(`/api/v1/products/${productId}/dislike`,
        userId, {
...
export const trackOrder = async (orderId, token) => {
    const res = await instance.get(`/api/v1/orders/${orderId}/tracking`, {
        headers: { Authorization: `Bearer ${token}` },
    })
    return res
}
...
export const createNewProduct = async (product, token) => {
    const res = await instance.post('/api/v1/products/create',
        product,
        {
            headers: { Authorization: `Bearer ${token}` }
        }
